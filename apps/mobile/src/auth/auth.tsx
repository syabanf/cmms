import type { Person, Role, Site } from '@cmms/types'
import { type ReactNode, createContext, useContext, useMemo, useState } from 'react'
import { Navigate, useLocation } from 'react-router'
import { readStorage, writeStorage } from '../lib/storage'
import { useStore } from '../state/store'

const SESSION_KEY = 'cmms.mobile.session'

/** Roles this app serves. Planners, supervisors and managers work in the maintenance console. */
const MOBILE_ROLES: Role[] = ['technician', 'requester']
...
interface AuthValue {
  user: Person | null
  /** Technicians and requesters belong to one site. */
  site: Site | null
...
export function RequireAuth({ children }: { children: ReactNode }) {
  ...
  if (!user || !site) return <Navigate to="/login" replace state={{ from: location.pathname + location.search }} />
  return children
}

/** Work lists belong to technicians; requesters land on their home screen instead. */
